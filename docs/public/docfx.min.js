...
`)})}function Xo(i){localStorage.setItem("theme",i),i==="auto"?document.documentElement.setAttribute("data-bs-theme",window.matchMedia("(prefers-color-scheme: dark)").matches?"dark":"light"):document.documentElement.setAttribute("data-bs-theme",i)}async function Zo(){return localStorage.getItem("theme")||(await D()).defaultTheme||"auto"}async function Co(){Xo(await Zo())}function Go(i){return new MutationObserver(()=>i(xe())).observe(document.documentElement,{attributes:!0,attributeFilter:["data-bs-theme"]})}function xe(){return document.documentElement.getAttribute("data-bs-theme")}async function Bo(i){let t=await Zo(),c=t==="light"?"sun":t==="dark"?"moon":"circle-half";return R`
    <div class='dropdown'>
      ...
      <ul class='dropdown-menu dropdown-menu-end'>
        ...
        </div>`;return R`${X} ${g()}`}function g(){let r=Math.max(0,Math.min(d-2,s-5)),Z=Math.min(s,r+5),X=Array.from(new Array(Z-r).keys()).map(G=>G+r);if(X.length<=1)return null;return R`
        <nav>
          <ul class="pagination">
            <li class="page-item">
        ...
        </nav>`;function B(G){G>=0&&G<s&&a(I,G)}}}}function pe(i,t){let e=t.split(/\s+/g).map(a=>a.toLowerCase()),b=i.toLowerCase(),o=[],l=0;for(let a=0;a<e.length;a++){let I=e[a],d=b.indexOf(I,l);d>=0&&(o.push(R`${i.slice(l,d)}`),o.push(R`<b>${i.slice(d,d+I.length)}</b>`),l=d+I.length)}return o.push(R`${i.slice(l)}`),R`${o}`}async function Ro(){let i=at("docfx:tocrel");if(!i)return[];let t=at("docfx:disabletocfilter")==="true",c=new URL(i.replace(/.html$/gi,".json"),window.location.href),{items:e,pdf:b,pdfFileName:o}=await(await fetch(c)).json(),l=t?"":localStorage?.getItem("tocFilterUrl")||"",a=t?"":localStorage?.getItem("tocFilter")||"";(l===""||l!==c.toString())&&(a="",t||(localStorage?.setItem("tocFilter",""),localStorage?.setItem("tocFilterUrl",c.toString())));let I=[],d=[];e.forEach(s);let n=document.getElementById("toc");if(n){C();let X=n.querySelectorAll("li.active"),B=X[X.length-1];B&&(B.scrollIntoView({block:"nearest"}),location.hash&&(location.href=location.href))}return d.length>0&&Jd(e,d[0]),I.slice(0,-1);function s(X){let B;if(X.href){let G=new URL(X.href,c);X.href=G.href,B=ui(G)?!1:Fo(G,window.location),B&&(X.items&&(X.expanded=!0),d.push(X))}if(X.items)for(let G of X.items)s(G)&&(B=!0,X.expanded=!0);return B?(I.unshift(X),!0):!1}function C(){lt(R`
      ${F()} 
      <div class="flex-fill overflow-y-auto">${g(e)||r()}</div>
      ${Z()}`,n)}function g(X){let B=X.map(G=>{let{href:u,name:x,items:Q,expanded:p}=G;if(!x)return null;let h=!Q||Q.length<=0,f=h?null:g(Q);if(a!==""&&!f&&!x.toLowerCase().includes(a.toLowerCase()))return null;let y=u?R`<a class='${ht({"nav-link":!I.includes(G)})}' href=${u}>${it(x)}</a>`:h?R`<span class='text-body-tertiary name-only'>${it(x)}</a>`:R`<a class='${ht({"nav-link":!I.includes(G)})}' href='#' @click=${T}>${it(x)}</a>`,v=a!==""&&p!==!1&&f!=null||p===!0;return R`
      ...
      </form>`;function X(B){a=B.target.value.trim(),localStorage?.setItem("tocFilter",a),C()}}function r(){return a===""?null:R`<div class='no-result'>${L("searchNoResults",{query:a})}</div>`}function Z(){return b?R`<div class="py-2 mb-md-4"><a class="pdf-link" href="${new URL(o||"toc.pdf",c)}">${L("downloadPdf")}</a></div>`:null}}function Jd(i,t){let c=document.getElementById("nextArticle");if(!c)return;let e=d(i),b=e.findIndex(n=>n===t),o=e[b-1],l=e[b+1];if(!o&&!l)return;let a=o?R`<div class="prev"><span><i class='bi bi-chevron-left'></i> ${L("prevArticle")}</span> <a href="${o.href}" rel="prev">${it(o.name)}</a></div>`:null,I=l?R`<div class="next"><span>${L("nextArticle")} <i class='bi bi-chevron-right'></i></span> <a href="${l.href}" rel="next">${it(l.name)}</a></div>`:null;lt(R`${a} ${I}`,c);function d(n){let s=[];for(let C of n)C.href&&s.push(C),C.items&&s.push(...d(C.items));return s}}var po=No(Qo());async function ho(){let i=document.getElementById("navbar");if(!i)return[];let{iconLinks:t}=await D(),c=await a(),e=wd(c),b=I=>R`<li class='nav-item'><a class='nav-link ${I===e?"active":null}' aria-current=${I===e?"page":!1} href=${I.href}>${it(I.name)}</a></li>`,o=R`
    <ul class='navbar-nav'>${c.map(I=>{if("items"in I){let d=I.items.some(n=>n===e)?"active":null;return R`
      ...
      <form class="icons">
        ...
        ${await Bo(l)}
      </form>`;lt(R`${o} ${I}`,i)}return await l(),e?[e]:[];async function a(){let I=at("docfx:navrel");if(!I)return[];let d=new URL(I.replace(/.html$/gi,".json"),window.location.href),{items:n}=await fetch(d).then(s=>s.json());return n.map(s=>"items"in s?{name:s.name,items:s.items.map(C=>({name:C.name,href:new URL(C.href,d)}))}:{name:s.name,href:new URL(s.href,d)})}}function Vo(i){let t=document.getElementById("breadcrumb");t&&lt(R`
        <ol class="breadcrumb">
